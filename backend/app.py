...
@app.route('/')
def home():
    return jsonify({"message": "Flask backend running with PostgreSQL on Render"})
...
def login():
    ...
    if user and check_password_hash(user[3], password):
        ...
        return jsonify({'token': token, 'name': user[1], 'email': user[2]}), 200
    else:
        return jsonify({'error': 'Invalid credentials'}), 401


# --- Products route (sample) ---
...
def products():
    ...
    products = [
        {"id": r[0], "name": r[1], "price": r[2], "category": r[3]} for r in rows
    ]
